...
# для сетевого варианта игра должна разворачиваться у каждого на компе (запуск)
# при этом можно добавить специальный код для проверки
# после подключения (должна быть надпись, что оно активно)
# каждый выбирает персонажа и нажимает пробел (энтер?)
# далее идёт ожидание всех игроков, когда все подключились -> энтер (игроки сами регульруют все ли подключились)
# общий сервер получает информацию о персонаже (id), его картинке и координатах 
# и расслыает остальным
# + он создаёт препятствия и рассылает их координаты (? рассылать только наальные координаты 
# и считать локально или рассылать на каждом кадре - лок только отрисовывать)
# на локальных компьютерах происходит отрисовка персонажей по id, картинкам и координатам
# игра для каждого заканчивается (при столкновении) и после _его_ игра заканчивается,
# сервер не рассылает его координаты, он не отрисовывается, но может наблюдать за остальными
# игра целиком заканчивается после последнего игрока
# на экране отображается премя пребывания для всех игроков по убыванию
# по нажатию на пробел - переход в лобби (ожидание новой игры)
# * у кого-то на платформе был проект по сети, нужно поискать
